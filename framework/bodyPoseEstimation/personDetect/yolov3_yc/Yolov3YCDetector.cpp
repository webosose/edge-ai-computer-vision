...
t_aif_status Yolov3YCDetector::fillInputTensor(const cv::Mat &img) /* override*/
{
    try {
        ...
        if (m_interpreter == nullptr) {
            throw std::runtime_error("yolov3_yc.tflite interpreter not initialized!!");
        }
...
t_aif_status Yolov3YCDetector::preProcessing()
{
    try
    {
        ...
        if (param == nullptr) {
            throw std::runtime_error("failed to convert DetectorParam to Yolov3YCParam");
        }
...
t_aif_status Yolov3YCDetector::postProcessing(const cv::Mat &img, std::shared_ptr<Descriptor> &descriptor)
{
    try {
        ...
        if (yolov3Descriptor == nullptr) {
            throw std::runtime_error("failed to convert Descriptor to Yolov3YCDescriptor");
        }
...
void Yolov3YCDetector::Classify_OBD_Result(const std::vector<BBox> &bbox, int num_nms, std::vector<int> &res_nms)
{
    m_obd_result = {0,};
    m_bodyResult = {0,};
    m_faceResult = {0,};
...
bool Yolov3YCDetector::checkUpdate(int index, const BBox &currBbox)
{
    std::shared_ptr<Yolov3YCParam> param = std::dynamic_pointer_cast<Yolov3YCParam>(m_param);
    if (param == nullptr) {
        Loge(__func__, "failed to convert DetectorParam to Yolov3YCParam");
