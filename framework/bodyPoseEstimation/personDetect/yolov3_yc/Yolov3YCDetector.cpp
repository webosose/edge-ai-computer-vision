...
constexpr int Yolov3YCDetector::tcnt_init[];

Yolov3YCDetector::Yolov3YCDetector(const std::string &modelPath)
...
Yolov3YCDetector::~Yolov3YCDetector()
{
}

std::shared_ptr<DetectorParam> Yolov3YCDetector::createParam()
{
    std::shared_ptr<DetectorParam> param = std::make_shared<Yolov3YCParam>();
    return param;
}

void Yolov3YCDetector::setModelInfo(TfLiteTensor *inputTensor)
{
...
t_aif_status Yolov3YCDetector::fillInputTensor(const cv::Mat &img) /* override*/
{
    ...
    if (m_interpreter == nullptr) {
        Loge(__func__, " yolov3_yc.tflite interpreter not initialized!!");
        return kAifError;
    }
...
t_aif_status Yolov3YCDetector::preProcessing()
{
    std::shared_ptr<Yolov3YCParam> param = std::dynamic_pointer_cast<Yolov3YCParam>(m_param);
    if (param == nullptr) {
        Loge(__func__, " failed to convert DetectorParam to Yolov3YCParam");
        return kAifError;
    }
    ...
    if (boost::iequals(param->detectObject, "body")) {
        ...
        Loge(__func__, " invalid param, not body nor face");
        return kAifError;
    }
...
t_aif_status Yolov3YCDetector::postProcessing(const cv::Mat &img, std::shared_ptr<Descriptor> &descriptor)
{
    Stopwatch sw;
    sw.start();
    ...
    std::shared_ptr<Yolov3YCDescriptor> yolov3Descriptor = std::dynamic_pointer_cast<Yolov3YCDescriptor>(descriptor);
    if (yolov3Descriptor == nullptr) {
        Loge(__func__, " failed to convert Descriptor to Yolov3YCDescriptor");
        return kAifError;
    }
    ...
    for (unsigned int i = 0; (i < finalBboxList.size()) && (i < param->numMaxPerson); i++) {
...
void Yolov3YCDetector::getPaddedImage(const cv::Mat &src, cv::Mat &dst)
{
...
void Yolov3YCDetector::transform2OriginCoord(const cv::Mat &img, t_pqe_obd_result &result)
{
...
void Yolov3YCDetector::OBD_ComputeResult(std::vector<unsigned short>& obd_lb_box_addr, std::vector<unsigned short>& obd_lb_conf_addr, std::vector<unsigned short>& obd_mb_box_addr,
...
void Yolov3YCDetector::CV_BOX(BoxType boxType, std::vector<unsigned short>& obd_addr, std::vector<unsigned short>& obd_conf, std::vector<BBox> &bbox)
{
    std::shared_ptr<Yolov3YCParam> param = std::dynamic_pointer_cast<Yolov3YCParam>(m_param);
    if (param == nullptr) {
        Loge(__func__, "failed to convert DetectorParam to Yolov3YCParam");
    ...
    int j, i, c;
    unsigned int cnt_box, cnt_conf;
    unsigned int BX0, BY0, BX1, BY1; // >= 0
    int CO;
    int BX_partial, BY_partial;
    int box_h, box_w, partial_shift, box_size;
    unsigned int boxIdx;
    size_t numBox_before = bbox.size();
    ...
    for (j = 0; j < box_h; j++) {
        ...
        for (i = 0; i < box_w; i++) {
            ...
            for (c = 0; c < box_size; c++) {
                ...
                int max_id = -1;
                for (unsigned int n = 0; n < param->class_size; n++) {
...
std::vector<int> Yolov3YCDetector::fw_nms(std::vector<BBox> &bbox)
{
    std::vector<int> res_nms;
    std::shared_ptr<Yolov3YCParam> param = std::dynamic_pointer_cast<Yolov3YCParam>(m_param);
    if (param == nullptr) {
...
void Yolov3YCDetector::Classify_OBD_Result(const std::vector<BBox> &bbox, unsigned int num_nms, std::vector<int> &res_nms)
{
    m_obd_result = {0,};
    m_bodyResult = {0,};
    m_faceResult = {0,};
    ...
    for (idx = 0; idx < num_nms; idx++) {
        ...
        if (selState != 0 && (m_obd_result.res_cnt >= 0 && m_obd_result.res_cnt < OBD_RESULT_NUM)) {
        ...
        {
            if (_class == eOBD_PERSON &&
                        (m_bodyResult.res_cnt >= 0 && m_bodyResult.res_cnt < OBD_RESULT_NUM)) {
            ...
            } else if (_class == eOBD_FACE &&
                        (m_faceResult.res_cnt >= 0 && m_faceResult.res_cnt < OBD_RESULT_NUM)) {
...
void Yolov3YCDetector::FaceMatching()
{
    std::shared_ptr<Yolov3YCParam> param = std::dynamic_pointer_cast<Yolov3YCParam>(m_param);
    if (param == nullptr) {
        Loge(__func__, "failed to convert DetectorParam to Yolov3YCParam");
...
bool Yolov3YCDetector::checkUpdate(unsigned int index, const BBox &currBbox)
{
    std::shared_ptr<Yolov3YCParam> param = std::dynamic_pointer_cast<Yolov3YCParam>(m_param);
    if (param == nullptr) {
        Loge(__func__, "failed to convert DetectorParam to Yolov3YCParam");
    ...
    if (m_prevBboxList.size() == 0 || m_prevBboxList.size() <= index) {
        Logd("UPDATE!!!!!!!!!!!!!!!!!");
        return true;
    }
...
void Yolov3YCDetector::RD_Result_box(std::vector<unsigned short>& obd_box, unsigned int* addr,unsigned int size_x, unsigned int size_y)
{
    int ix,iy,ic;
    unsigned int data;

    if (obd_box.size() < size_y * size_x * 12) {
        Loge(__func__, " obd_box size is too smaller than ", size_y * size_x * 12);
        return;
    }
    //Loge(__func__, " obd_box size: ", obd_box.size(), " = " , size_y * size_x * 12);
...
void Yolov3YCDetector::RD_Result_conf(std::vector<unsigned short>& obd_conf, unsigned int* addr,unsigned int size_x, unsigned int size_y)
{
    int ix,iy,ic;
    unsigned int data;

    if (obd_conf.size() < size_y * size_x * 33) {
        Loge(__func__, " obd_conf size is too smaller than ", size_y * size_x * 33);
        return;
    }
    //Loge(__func__, " obd_conf size: ", obd_conf.size(), " = " , size_y * size_x * 33);
...
void Yolov3YCDetector::RD_Result_box_SB(std::vector<unsigned short>& obd_box, unsigned int* addr,unsigned int size_x, unsigned int size_y)
{
    int ix,iy,ic;
	unsigned int data;
    ...
    if (obd_box.size() < size_y * size_x * 4) {
        Loge(__func__, " obd_box size is too smaller than ", size_y * size_x * 4);
        return;
    }
    //Loge(__func__, " obd_box size: ", obd_box.size(), " = " , size_y * size_x * 4);
...
void Yolov3YCDetector::RD_Result_conf_SB(std::vector<unsigned short>& obd_conf,unsigned int* addr,unsigned int size_x, unsigned int size_y)
{
    int ix,iy,ic;
	unsigned int data;

    if (obd_conf.size() < size_y * size_x * 11) {
        Loge(__func__, " obd_conf size is too smaller than ", size_y * size_x * 11);
        return;
    }
    //Loge(__func__, " obd_conf size: ", obd_conf.size(), " = " , size_y * size_x * 11);
