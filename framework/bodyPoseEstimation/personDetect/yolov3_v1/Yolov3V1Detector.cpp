...
t_aif_status Yolov3V1Detector::postProcessing(const cv::Mat& img, std::shared_ptr<Descriptor>& descriptor)
{
    try {
        ...
        std::shared_ptr<Yolov3V1Descriptor> yolov3V1Descriptor = std::dynamic_pointer_cast<Yolov3V1Descriptor>(descriptor);
        if (yolov3V1Descriptor == nullptr) {
            ...
            if (m_IsBodyDetect) {
                if (m_GTBBoxes.size() > m_frameId) {
                    ...
                    if (gtBBox.second.width > 0 && gtBBox.second.height > 0) {
                        yolov3V1Descriptor->addPerson(score, gtBBox.second, gtBBox.first);
                        Logi(__func__, " use GT Box !! ", gtBBox.first, ", ", gtBBox.second);
                    }

                } else {
                    yolov3V1Descriptor->addPerson(score, finalBbox);
                }
            } else {
                /* normalize 0~1 */
                yolov3V1Descriptor->addFace(score,
...
void Yolov3V1Detector::OBD_ComputeResult(std::vector<unsigned short>& obd_lb_box_addr, std::vector<unsigned short>& obd_lb_conf_addr, std::vector<unsigned short>& obd_mb_box_addr,
...
void Yolov3V1Detector::CV_BOX(BoxType boxType, std::vector<unsigned short>& obd_addr, std::vector<unsigned short>& obd_conf, std::vector<BBox> &bbox)
{
    std::shared_ptr<Yolov3V1Param> param = std::dynamic_pointer_cast<Yolov3V1Param>(m_param);
    if (param == nullptr) {
    ...
    int j,i,c,cnt,cnt_box,cnt_conf;
...
void Yolov3V1Detector::RD_Result_box(std::vector<unsigned short>& obd_box, unsigned int* addr,unsigned int size_x, unsigned int size_y)
{
    int ix,iy,ic,i;
    unsigned int data;
...
void Yolov3V1Detector::RD_Result_conf(std::vector<unsigned short>& obd_conf, unsigned int* addr,unsigned int size_x, unsigned int size_y)
{
    int ix,iy,ic,i;
    unsigned int data;
...
void Yolov3V1Detector::RD_Result_box_SB(std::vector<unsigned short>& obd_box, unsigned int* addr,unsigned int size_x, unsigned int size_y)
{
    int ix,iy,ic,i;
    unsigned int data;
...
void Yolov3V1Detector::RD_Result_conf_SB(std::vector<unsigned short>& obd_conf,unsigned int* addr,unsigned int size_x, unsigned int size_y)
{
    int ix,iy,ic,i;
    unsigned int data;
