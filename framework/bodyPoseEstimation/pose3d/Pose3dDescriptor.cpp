...
void Pose3dDescriptor::addJointsAndTraj3D(const std::vector<Joint3D> &joints3d, const Joint3D &joint3dPos)
{
...
void Pose3dDescriptor::addTraj3D(const Joint3D &joint3dPos)
{
    rj::Document::AllocatorType& allocator = m_root.GetAllocator();
    if (!m_root.HasMember("poses3d")) {
        rj::Value poses3d(rj::kArrayType);
