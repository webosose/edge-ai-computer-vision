...
NpuPose3dDetectorV2::NpuPose3dDetectorV2(const std::string& modelPath)
    : Detector(modelPath)
    , mMaxInputs(0)
    , mBatchSize(0)
    , mNumElems(0)
...
NpuPose3dDetectorV2::~NpuPose3dDetectorV2()
{
...
Joint2D NpuPose3dDetectorV2::normalizeJoints(const float x, const float y, const int width, const int height )
{
    Joint2D p;
...
t_aif_status NpuPose3dDetectorV2::detect(const cv::Mat &img, std::shared_ptr<Descriptor> &descriptor)
{
    try {
        mIsSecondDetect = false;
...
std::shared_ptr<DetectorParam> NpuPose3dDetectorV2::createParam()
{
    std::shared_ptr<DetectorParam> param = std::make_shared<Pose3dParam>();
    return param;
}

void NpuPose3dDetectorV2::setModelInfo(TfLiteTensor* inputTensor)
{
...
t_aif_status NpuPose3dDetectorV2::fillInputTensor(const cv::Mat& joints_mat)/* override*/
{
    try {
...
t_aif_status NpuPose3dDetectorV2::preProcessing()
{
    try {
        initializeParam();
...
t_aif_status NpuPose3dDetectorV2::postProcessing(const cv::Mat& img, std::shared_ptr<Descriptor>& descriptor)
{
    try {
        ...
        if (!mIsSecondDetect) postProcess_forFirstBatch(outputIdx, output);
        else {
            ...
            if(outputIdx == 0) pose3dDescriptor->addJoints3D(mResults[RESULT_3D_JOINT]);
...
void NpuPose3dDetectorV2::initializeParam()
{
    std::shared_ptr<Pose3dParam> param = std::dynamic_pointer_cast<Pose3dParam>(m_param);
    if (param == nullptr) {
        Loge(__func__, "failed to convert DetectorParam to Pose3dParam");
...
void NpuPose3dDetectorV2::getCameraIntrinsics()
{
    std::shared_ptr<Pose3dParam> param = std::dynamic_pointer_cast<Pose3dParam>(m_param);
    if (param == nullptr) {
        Loge(__func__, "failed to convert DetectorParam to Pose3dParam");
...
void NpuPose3dDetectorV2::getInputTensorInfo(TfLiteTensor *input)
{
...
int NpuPose3dDetectorV2::getOutputTensorInfo(TfLiteTensor *output)
{
...
void NpuPose3dDetectorV2::fillJoints(uint8_t* inputTensorBuff )
{
    TRACE(__func__);
...
void NpuPose3dDetectorV2::fillFlippedJoints(uint8_t* inputTensorBuff)
{
    TRACE(__func__);
...
void NpuPose3dDetectorV2::postProcess_forFirstBatch(int outputIdx, TfLiteTensor* output)
{
...
void NpuPose3dDetectorV2::postProcess_forSecondBatch(int outputIdx, TfLiteTensor* output)
{
    TRACE(__func__, "outputIdx: " , outputIdx, " alloc & memcpy with ", output->bytes, " bytes");
    if (outputIdx < 0 || outputIdx >= RESULT_3D_IDX_MAX) {
        Loge(__func__, "outputInx is out of range: ", outputIdx);
...
void NpuPose3dDetectorV2::averageWithFilippedResult(int idx, uint8_t* buff, uint8_t* flipped, int numInputs, int numJoints)
{
