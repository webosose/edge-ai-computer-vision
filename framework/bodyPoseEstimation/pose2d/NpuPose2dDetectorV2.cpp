...
NpuPose2dDetectorV2::NpuPose2dDetectorV2(const std::string& modelName)
    : mScaleIn(0.0f)
    , mZeropointIn(0)
...
NpuPose2dDetectorV2::~NpuPose2dDetectorV2() {}

void NpuPose2dDetectorV2::setModelInfo(TfLiteTensor* inputTensor)
{
...
t_aif_status NpuPose2dDetectorV2::fillInputTensor(const cv::Mat& img)/* override*/
{
    try {
...
t_aif_status NpuPose2dDetectorV2::preProcessing()
{
...
t_aif_status NpuPose2dDetectorV2::postProcessing(const cv::Mat& img, std::shared_ptr<Descriptor>& descriptor)
{
    Stopwatch sw;
    sw.start();
...
void NpuPose2dDetectorV2::getInputTensorInfo(TfLiteTensor *input)
{
...
void NpuPose2dDetectorV2::normalizeImageWithQuant(cv::Mat& img, cv::Mat& normImg) const
{
