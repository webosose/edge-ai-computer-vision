...
TfLiteGpuInferenceUsage stringToInferencePreference(const std::string& preference)
{
...
TfLiteGpuInferencePriority stringToPriority(const std::string& priority)
{
    ...
    Loge("Not exist priority option: ", priority, "aaaa");
    return TFLITE_GPU_INFERENCE_PRIORITY_AUTO;
}
...
void GpuDelegate::setupOptions()
{
    ...
    m_delegateOptions.inference_preference = TFLITE_GPU_INFERENCE_PREFERENCE_FAST_SINGLE_ANSWER;

    if (payload.HasMember("inference_preference")) {
        const rj::Value& preference = payload["inference_preference"];
        m_delegateOptions.inference_preference = stringToInferencePreference(preference.GetString());
    }
    if (payload.HasMember("inference_priorities")) {
        ...
        if (priorities.IsArray() && priorities.Size() == 3) {
            m_delegateOptions.inference_priority1 = stringToPriority(priorities[0].GetString());
            m_delegateOptions.inference_priority2 = stringToPriority(priorities[1].GetString());
            m_delegateOptions.inference_priority3 = stringToPriority(priorities[2].GetString());
            Logi("inference priority1: ", priorities[0].GetString(), " : ", m_delegateOptions.inference_priority1);
            Logi("inference priority2: ", priorities[1].GetString(), " : ", m_delegateOptions.inference_priority2);
            Logi("inference priority3: ", priorities[2].GetString(), " : ", m_delegateOptions.inference_priority3);
        } else {
            Loge("Failed to set inference priorites: ",
                    priorities[0].GetString(),
                    priorities[1].GetString(),
                    priorities[2].GetString());
        }
    }
    if (payload.HasMember("enable_quntized_inference")) {
        if (payload["enable_quntized_inference"].GetBool()) {
            m_delegateOptions.experimental_flags |= TFLITE_GPU_EXPERIMENTAL_FLAGS_ENABLE_QUANT;
        } else {
            m_delegateOptions.experimental_flags &= ~TFLITE_GPU_EXPERIMENTAL_FLAGS_ENABLE_QUANT;
        }
        Logi("enabled quntized inference : ", payload["enable_quntized_inference"].GetBool());
    }
    if (payload.HasMember("backends")) {
        const rj::Value& backends= payload["backends"];
        if (backends.IsArray() && backends.Size() > 0) {
            if (backends[0].GetString() == "cl") {
                m_delegateOptions.experimental_flags |= TFLITE_GPU_EXPERIMENTAL_FLAGS_CL_ONLY;
            } else if (backends[0].GetString() == "gl") {
                m_delegateOptions.experimental_flags |= TFLITE_GPU_EXPERIMENTAL_FLAGS_GL_ONLY;
            }
    ...
    if (payload.HasMember("max_delegated_partitions")) {
        m_delegateOptions.max_delegated_partitions = payload["max_delegated_partitions"].GetInt();
    }
}
...
TfLiteDelegatePtr GpuDelegate::getTfLiteDelegate() const
{
    return TfLiteDelegatePtr(
            TfLiteGpuDelegateV2Create(&m_delegateOptions),
