...
public:
    FaceDetectSample(
        ...
        :ImageSample(configPath, defaultConfig, true) {}
...
protected:
    virtual const cv::Mat& readImage() override 
    {
        ...
        m_image.copyTo(padded_image(cv::Rect(0, 0, m_image.cols, m_image.rows)));
    ...
    virtual void onResult(std::shared_ptr<Descriptor>& descriptor)
    {
        ...
        for (rj::SizeType i = 0; i < faces.Size(); i++) {
            ...
            std::cout << i << ": [ " << rect.x << ", " << rect.y << ", " << rect.width 
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 2) {
        if (!std::strcmp(argv[1], "--h") || !std::strcmp(argv[1], "--help")) {
            std::cout << "Usage: face-detect-sample <config-file-path>" << std::endl;
