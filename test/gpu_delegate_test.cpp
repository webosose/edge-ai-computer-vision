...
protected:
    ...
    std::string options2{
        "{"
            ...
            " \"inference_priorities\": [\"MAX_PRECISION\", \"MIN_MEMORY_USAGE\", \"AUTO\"], "
...
TEST_F(GpuDelegateTest, 02_constructor_with_options1)
{
    ...
    EXPECT_EQ(option.inference_preference, TFLITE_GPU_INFERENCE_PREFERENCE_SUSTAINED_SPEED);
    EXPECT_EQ(option.inference_priority1, TFLITE_GPU_INFERENCE_PRIORITY_MIN_LATENCY);
    EXPECT_EQ(option.inference_priority2, TFLITE_GPU_INFERENCE_PRIORITY_MIN_MEMORY_USAGE);
    EXPECT_EQ(option.inference_priority3, TFLITE_GPU_INFERENCE_PRIORITY_MAX_PRECISION);
    EXPECT_EQ(option.experimental_flags & TFLITE_GPU_EXPERIMENTAL_FLAGS_ENABLE_QUANT,
...
TEST_F(GpuDelegateTest, 02_constructor_with_options2)
{
    ...
    std::shared_ptr<GpuDelegate> gpuDelegate =
        std::dynamic_pointer_cast<GpuDelegate>(delegate);
    auto option = gpuDelegate->getDelegateOptions();
    EXPECT_EQ(option.inference_preference, TFLITE_GPU_INFERENCE_PREFERENCE_FAST_SINGLE_ANSWER);
    EXPECT_EQ(option.inference_priority1, TFLITE_GPU_INFERENCE_PRIORITY_MAX_PRECISION);
    EXPECT_EQ(option.inference_priority2, TFLITE_GPU_INFERENCE_PRIORITY_MIN_MEMORY_USAGE);
    EXPECT_EQ(option.inference_priority3, TFLITE_GPU_INFERENCE_PRIORITY_AUTO);
    EXPECT_EQ(option.experimental_flags & TFLITE_GPU_EXPERIMENTAL_FLAGS_ENABLE_QUANT, 0);
    EXPECT_EQ(option.experimental_flags & TFLITE_GPU_EXPERIMENTAL_FLAGS_GL_ONLY,
