...
PalmParam::PalmParam()
    : strides{8, 16, 16, 16}, optAspectRatios{1.0f},
      interpolatedScaleAspectRatio(1.0f), anchorOffsetX(0.5),
      anchorOffsetY(0.5), minScale(0.1484375), maxScale(0.75),
      reduceBoxesInLowestLayer(false), scoreThreshold(0.5f), iouThreshold(0.2f),
      maxOutputSize(100), updateThreshold(0.3f) {}

PalmParam::~PalmParam() {}

PalmParam::PalmParam(const PalmParam &other)
    : strides{other.strides}, optAspectRatios{other.optAspectRatios},
      interpolatedScaleAspectRatio{other.interpolatedScaleAspectRatio},
      anchorOffsetX{other.anchorOffsetX}, anchorOffsetY{other.anchorOffsetY},
      minScale{other.minScale}, maxScale{other.maxScale},
      reduceBoxesInLowestLayer{other.reduceBoxesInLowestLayer},
      scoreThreshold{other.scoreThreshold}, iouThreshold{other.iouThreshold},
      maxOutputSize{other.maxOutputSize}, updateThreshold{
                                              other.updateThreshold} {
    // TRACE(TAG, "COPY CONSTRUCTOR....");
}

PalmParam::PalmParam(PalmParam &&other) noexcept
    : strides{std::move(other.strides)}, optAspectRatios{std::move(
                                             other.optAspectRatios)},
      interpolatedScaleAspectRatio{
          std::exchange(other.interpolatedScaleAspectRatio, 1.0f)},
      ...
      reduceBoxesInLowestLayer{
          std::exchange(other.reduceBoxesInLowestLayer, false)},
...
bool PalmParam::operator==(const PalmParam &other) const {
    return (
        ...
        (std::equal(optAspectRatios.begin(), optAspectRatios.end(),
                    other.optAspectRatios.begin())) &&
        (std::abs(interpolatedScaleAspectRatio -
                  other.interpolatedScaleAspectRatio) < aif::EPSILON) &&
...
std::ostream &operator<<(std::ostream &os, const PalmParam &fp) {
    ...
    os << "\treduceBoxesInLowestLayer: "
       << (fp.reduceBoxesInLowestLayer ? "true" : "false") << ",\n";
...
t_aif_status PalmParam::fromJson(const std::string &param) {
    ...
    if (payload.HasMember("modelParam")) {
        ...
        if (modelParam.HasMember("optAspectRatios")) {
            ...
            for (auto &optAspectRatio :
                 modelParam["optAspectRatios"].GetArray()) {
        ...
        if (modelParam.HasMember("interpolatedScaleAspectRatio"))
            interpolatedScaleAspectRatio =
                modelParam["interpolatedScaleAspectRatio"].GetFloat();
        ...
        if (modelParam.HasMember("reduceBoxesInLowestLayer"))
            reduceBoxesInLowestLayer =
                modelParam["reduceBoxesInLowestLayer"].GetBool();
